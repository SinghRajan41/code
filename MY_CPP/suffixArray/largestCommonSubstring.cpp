// The first task is: given a string s, you need to find the number of its different substrings.
#include <iostream>
#include <algorithm>
#include <string>
#include <vector>
...
vector<int> buildLCP(vector<int> &p, string s);
int main()
{
   ...
   string a = s + '#' + t;
   int n = a.size() + 1;
   vector<int> p = buildSuffix(a);
   vector<int> lcp = buildLCP(p, a);
   vector<int> type(n);
   int len = 0;
   int idx = 0;
   for (int i = 0; i < n; i++)
...
vector<int> buildLCP(vector<int> &p, string s)
{
   ...
   for (int i = 0; i < n; i++)
   {
      int x = pos[i];
      int y = x - 1;
      while ((p[x] + k < n) && y >= 0 && (p[y] + k < n) && (s[p[x] + k] == s[p[y] + k]))
         k++;
      lcp[x] = k;
      k = max(k - 1, 0);
   }
