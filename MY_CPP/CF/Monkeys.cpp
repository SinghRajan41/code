...
class dsu
{
    ...
    int findParent(int a){if(a==parent[a])return a;else {int temp = findParent(parent[a]);parent[a] = temp;return temp;}}
...
int main()
{
    ...
    for(int i=m;i>=1;i--)
    {
        ...
        if(b != -1)
        {
            a = d.findParent(a);b = d.findParent(b);
            if(a != 1)
            { for(auto x:d.f[a])  time[x] = i-1;}
            if(b != 1)
            { for(auto x:d.f[b])  time[x] = i-1;}
            d.Union(a,b);
        }
    }
