...
{
    int n,m;    cin>>n>>m;
    int arr[n+1];   for(int i=1;i<=n;i++)   cin>>arr[i];              //arr[i] stores the unhappines caused by removal of ith person from party
    int present[n+1]; for(int i=0;i<=n;i++)   present[i] = 0;
    vector<pair<int,int>> pairs(m , {-1 , -1});
    for(int i=0;i<m;i++)
    {
        int a,b;    cin>>a>>b;
        pairs[i] = {a,b};
    ...
    if(m&1)
    {
        int p,q;
        int ans = INT32_MAX;
        for(int i=1;i<=n;i++)   if(present[i] & 1)  {   ans = min(ans , arr[i]);    }
        for(int i=0;i<m;i++)
        {
            int a,b;     a = pairs[i].first;    b = pairs[i].second;
            int rem = m + 1 - (present[a] + present[b]);
            if(rem>=0 && rem%2==0)
            {
